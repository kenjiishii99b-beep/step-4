...
| 16 | UT-01 | 2026-09-12 | Claude Code（claude-in-chromeによる実機ブラウザ操作） | 一部確認 | - | Azure本番環境で、バーコードのカメラスキャン→カート追加→現金決済→レシート表示までの一連の流れを実機ブラウザ（スマートフォン想定のカメラ機能含む）で確認。本シナリオが含む「会員読取・割引適用」は同一操作フロー内では未実施（会員照会機能自体は`tests/test_members.py`等で別途検証済み）。 |
| 17 | UT-04 | 2026-09-12 | Claude Code（claude-in-chromeによる実機ブラウザ操作） | 合格 | - | Azure本番環境で、元取引ID照合→対象SKU・数量選択→サーバー金額再計算→返品確定までの一連の流れを実機ブラウザで確認。`parent_transaction_id`の設定・在庫復元（+1）・返金額のマイナス表示を確認。 |
